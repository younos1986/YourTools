...
using YourTools.Mapper.CodeGeneration.Extensions;
using YourTools.Mapper.CodeGeneration.Models;

namespace YourTools.Mapper.CodeGeneration.Analysis
{
                    ...
                    foreach (var statement in method.Body?.Statements ?? Enumerable.Empty<StatementSyntax>())
                    {
                            ...
                            exprStmt.Expression is AssignmentExpressionSyntax assignExpr)
                        {
                            if (assignExpr.Left is MemberAccessExpressionSyntax memberAccess &&
                                ...
                                memberAccess.Name.Identifier.Text == "EnableProjectionFunctions")
                            {
                                ...
                                {
                                    _configuration.EnableProjectionFunctions = literal.Kind() == SyntaxKind.TrueLiteralExpression;
                                }
                            }
                        }
                    }
