    ...
    public class MediatorHandlerGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            var isEnabledProvider = context.AnalyzerConfigOptionsProvider
                .Select((opts, _) =>
                {
                    opts.GlobalOptions.TryGetValue("build_property.EnableYourToolsMediatorGeneratedCode", out var value);
        ...
        {
            sb.AppendLine("using YourTools.Mediator;");
                    ...
                    )
            {
                if (!string.IsNullOrWhiteSpace(ns) &&
                    ns != "YourTools.Mediator" &&
                    ns != "YourTools.Mediator.Contracts" &&
        ...
        {
            sb.AppendLine("namespace YourTools.Mediator");
            sb.AppendLine("{");
        ...
        private static List<(string HandlerType, string RequestType, string ResponseType, string Namespace)> ScanReferencedAssembliesForRequestHandlers(Compilation compilation, HashSet<string> namespaces)
        {
            ...
            var iRequestSymbol = compilation.GetTypeByMetadataName("YourTools.Mediator.Contracts.IRequest`1");
            var iRequestHandlerSymbol = compilation.GetTypeByMetadataName("YourTools.Mediator.Contracts.IRequestHandler`2");
